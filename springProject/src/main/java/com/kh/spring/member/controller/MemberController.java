...
//		System.out.println(loginMember);
	...
	public String insertMember(Member m, HttpSession session, HttpServletRequest request) throws UnsupportedEncodingException {
		//비밀번호 확인은 어차피 HttpServletRequest를 이용해볼까? 
...
//	    System.out.println(request.getParameter("userPwd"));
		...
		m.setUserPwd(bcrypt.encode(m.getUserPwd())); // 비밀번호를 다시 암호화 시켜야함. (데이터 베이스로 넘어가기전에 암호화를 해둬야함)
		
		/*
		 * 어차피 memberEnrollForm.jsp 파일에서 제출 버튼을 눌렀을때 위 메서드가 실행되는 것이므로 
		 * 제출 버튼을 누르기전에는 원래 비밀번호(암호화 안된 비밀번호 = 평문)과 확인 비밀번호랑 비교
		 * 버튼을 누르면 바뀜. 
		 */
	...
	public String deleteMember(Member member, HttpSession session) {
		...
		String inputPwd = member.getUserPwd(); // 마이페이지에서 입력한 비밀번호 갖고오기 
		
		Member loginMember = (Member)session.getAttribute("loginMember"); // 로그인 한 멤버의 비밀번호는 복호화되어있음.
		
		//복호화된 비밀번호와 form태그에서 입력했던 비밀번호를 대조해야함.  (이것만 하면됨) 
		
		int result = 0;
		
		if(bcrypt.matches(inputPwd, loginMember.getUserPwd())) {
			result = service.deleteMember(loginMember); //이건 데이터베이스에서 없애는거고 암호화된 비밀번호랑 비교해봐야함.		    	
		} 
			
		//int result = service.deleteMember(loginMember); //이건 데이터베이스에서 없애는거고 암호화된 비밀번호랑 비교해봐야함. 
		
		if(result > 0) { //성공하면 로그아웃까지 시켜야함 복호화된 비밀번호랑 입력한 비밀번호랑 일치하면 회원 탈퇴
			
			session.setAttribute("alertMsg", "회원탈퇴 성공!");
			
			session.removeAttribute("loginMember"); // 이건 웹브라우저에서 없애기 
			
			return "redirect:/"; // 메인 페이지로 돌아가기
		
		} else { // 성공하지 못하면 복호화된 비밀번호랑 입력한 비밀번호랑 달라서 회원 탈퇴 실패
